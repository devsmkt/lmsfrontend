...
// Normalize API base URL: remove any trailing `/api` or trailing slash
...
export const getSEO = async ({
    ...
    const globalSeo = await fetch(apiBase + ENDPOINTS.SEO, {
        next: { revalidate: 300 }
    }).then((res) => res.json());
...
export const customPageData = cache(async (slug) => {
    try {
        ...
        // If the custom page is not found, return a sentinel value used by pages
        if (error?.response?.status === 404) {
            return { remark: 'page_not_found' };
        }
        throw error;
    }
});
...
export function getCourseImage(coursePath, courseImage) {
    ...
    const apiBase = (process.env.NEXT_PUBLIC_API_URL || '').replace(/\/api\/?$/,'').replace(/\/$/, '');
    return `${apiBase}/assets/images/default.png`;
}
